---
title: "Penalized global quantile regression using MM algorithm"
output:
  pdf_document: default
bibliography: alg_ref.bib
---
## Introduction
...
$$
where $\varphi_m$ is the $m$th *column* of $\boldsymbol{\varphi}$. Letting $\boldsymbol{\eta}$ denote an $N\times M$ matrix with positive entries, we can use the [eta-trick](https://francisbach.com/the-%CE%B7-trick-or-the-effectiveness-of-reweighted-least-squares/) which states that 

$$
2|v| = \min_{\eta > 0}\frac{v^2}{\eta}+\eta
$$

to write the above minimization problem alternatively as
$$
...
0. Initialize $\eta_{nm}^0$ and $\zeta^0$ arbitrarily.
1. Given the values of $\eta_{nm}^{r-1}$ and $\zeta^{r-1}$ in the (r-1)th iteration, find the solution $\widehat{\boldsymbol{a}}^{r}$ that minimizes $(2)$ w.r.t. $\boldsymbol{a}$ ;
2. Update $\eta_{nm}^r = |Y_n - vec(X_n \varphi_m')'vec(\boldsymbol{a})^r|$ and $\zeta = \Vert \boldsymbol{a} \Vert$;
3. Iterate steps 1-2 until the convergence criterion has been met: $\frac{abs(R_{\lambda}(\boldsymbol{a}^{r},\eta^{r},\zeta^{r}) - R_{\lambda}(\boldsymbol{a}^{r+1},\eta^{r+1},\zeta^{r+1}))}{R_{\lambda}(\boldsymbol{a}^{r},\eta^{r},\zeta^{r})} < tol$.

Note: Use a precision parameter $\pi$ to set $\eta_{nm}^{r} = \max\{\pi,\eta_{nm}^{r}\}$ and $\zeta^{r} = \max\{\pi,\zeta^{r}\}$.

## Additional notes

By vectorizing the matrix $\boldsymbol{a}$, we are able to find an analyitical solution to $vec(\boldsymbol{a})$ and use the standard `optim()` function to perform step 1:
...
In this notation, we differentiate $(2)$ and find the estimator for $vec(\boldsymbol{a})$ as follows: 
